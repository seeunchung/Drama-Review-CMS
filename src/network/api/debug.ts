...
export const debugApi = {
    ...
    seedAllData: async () => {
        ...
        await supabase.from("drama_comments").delete().neq("id", "00000000-0000-0000-0000-000000000000");
        ...
        const dramas = [
            { title: "장상사 (Lost You Forever)", status: "completed", file: "장상사_시즌1_메타데이터.xlsx" },
            { title: "투나망월 (The Double)", status: "completed", file: "투나망월_전회차.xlsx" },
            { title: "경여년 2 (Joy of Life 2)", status: "completed", file: "경여년_시즌2_최종.xlsx" },
            { title: "신조협려 2024", status: "pending", file: "신조협려_검토용.xlsx" },
            { title: "유리미인살", status: "completed", file: "유리미인살_재업로드.xlsx" },
            { title: "삼생삼세 십리도화", status: "failed", file: "삼생삼세_데이터오류.xlsx" },
        ];
        ...
        for (const d of dramas) {
            ...
            const { error: eError } = await supabase.from("episodes").insert(episodesData);
            ...
            if (d.status === "completed") {
                ...
                const nicknames = ["중드덕후", "시엔", "드라마조아", "망붕렌즈", "무협팬", "OTT수집가"];
                const comments = [
                    "진짜 인생작입니다ㅠㅠ", "여주 연기 미쳤네요", "남주 비주얼 실화인가요", 
                    "원작보다 잘 만든듯", "다음 화 언제 나와요", "방금 다 봤는데 여운이 가시질 않음",
                    "스토리 전개가 좀 느리지만 볼만함", "배경 음악이 너무 좋아요"
                ];

                const commentsData = Array.from({ length: count }).map(() => ({
                    drama_id: batch.id,
                    episode_no: Math.floor(Math.random() * 12) + 1,
                    user_nickname: nicknames[Math.floor(Math.random() * nicknames.length)],
                ...
                const { error: cError } = await supabase.from("drama_comments").insert(commentsData);
